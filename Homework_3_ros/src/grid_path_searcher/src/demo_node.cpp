...
{   
    ...
    ROS_INFO("demo_node received the planning target");
...
{   
    ...
    ROS_INFO("demo_node received the PointCloud");
    _has_map = true;
}
...
void pathFinding(const Vector3d start_pt, const Vector3d target_pt)
{
    ...
    // 将设置的边界赋予状态空间space，as的作用是类型转换
    space->as<ob::RealVectorStateSpace>()->setBounds(bounds);
    ROS_INFO("Bounds setted!");

    // Construct a space information instance for this state space
    ob::SpaceInformationPtr si(new ob::SpaceInformation(space));
    ROS_INFO("space information instance is constructed");
    // Set the object used to check which states in the space are valid
    si->setStateValidityChecker(ob::StateValidityCheckerPtr(new ValidityChecker(si)));
    si->setup();
    ROS_INFO("This instance is Setted");
    // Set our robot's starting state
    ob::ScopedState<> start(space);
    ROS_INFO("Start State Created");
    /**
    *
    *
    ...
    */ 
    ...
    ROS_INFO("STEP-6 Over");

    if (solved)
    {
        ROS_INFO("STEP-6:Solved successfullt!");
